...
현재가는 최신 `primary_candle` 종가 시세입니다.

| 주문 유형 | 실제 Trader (UPB/BTH/BNC/OKX) | `SimulationTrader` |
|-----------|:------------------------------:|:-------------------:|
| 시장가 | ✅ | ✅ 현재 종가 시세로 즉시 체결 |
| 지정가 | ✅ | ✅ 조건 충족까지 대기, 유리한 현재 종가 시세로 체결 |
| 매도 손절 | ❌ | ✅ 종가 시세가 trigger 이하일 때 체결 |
| 매도 익절 | ❌ | ✅ 종가 시세가 trigger 이상일 때 체결 |
| OCO | ❌ | ❌ |
| 트레일링 스톱 | ❌ | ❌ |

- **시장가/지정가**는 네 실거래 Trader에서 지원합니다. 요청에 `ord_type` 필드가 없으면 하위호환을 위해 지정가로 처리됩니다.
- **손절/익절**은 `SimulationTrader`의 매도 조건 주문입니다. 실제 Trader는 손절·익절 주문 유형을 지원하지 않고 명시적으로 거절합니다.

### 가상거래의 제한 사항

- 가상 잔고, 대기 주문, 주문 이력은 메모리에만 저장되므로 재시작하면 사라집니다.
- 대기 주문의 조건은 최신 `primary_candle` 종가 시세가 들어올 때만 평가됩니다. 캔들 진행 중 가격이 조건에 닿았다가 되돌아오면 감지되지 않을 수 있습니다.
- 가상 수수료는 0이며, 슬리피지, 부분 체결, 호가창 깊이, 거래소별 최소 주문 규칙, 호가 단위, 수량 정밀도는 반영하지 않습니다.
