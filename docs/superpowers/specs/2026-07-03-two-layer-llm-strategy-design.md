...
│   - get_portfolio / get_trade_history (읽기 전용, 기존 유지)       │
...
│    → trader.update_quote(currency, price)  # 가상매매 시세 주입    │
...
**`StrategyFactory`** — master의 factory를 복원·확장. 전략 코드(`BNH`/`RSI`/`SMA`/`LLM`) → 클래스 매핑 레지스트리. 프로파일의 `"strategy"` 필드 해석과 에이전트의 `list_strategies`/`select_strategy` Tool이 이 레지스트리를 사용한다.
...
- `get_request()` — `LlmClient`에 **구조화 판단**을 1회 요청하고 결과를 거래 요청 리스트로 변환. 프롬프트에는 시장 데이터 버퍼 + 내부 추적 잔고/보유 포지션 + 전략 지식을 포함해 LLM이 수량을 산정할 수 있게 한다.
- `update_result(result)` — 체결 결과로 내부 잔고/포지션을 갱신(알고리즘 전략과 동일한 자기-회계 패턴)하고, 다음 프롬프트 맥락에 반영.
...
- 틱: DataProvider → 시세 주입 → Strategy → (SafetyGuard) → Trader → Strategy.update_result → Analyzer

SafetyGuard 검사는 `Trader.send_request` 직전에 수행하여 **전략 종류와 무관하게** 하드리밋을 강제한다. `cancel` 타입 요청은 금액 검사 대상에서 제외한다.

**가상매매 시세 주입 (paper-trading 설계 승계)**: 매 틱 `DataProvider.get_info()` 직후, `primary_candle`의 `closing_price`를 추출해 `trader.update_quote(currency, price)`로 주입한다(덕 타이핑 — 실거래 트레이더에는 해당 메서드가 없어 no-op). 기존 `LlmOperator._sync_trader_quote` 로직을 이전한다. `SimulationTrader`의 체결가는 주입된 시세를 따른다.
...
- 읽기 전용 조회 Tool(`get_market_data`/`get_portfolio`/`get_trade_history`/`get_performance`)은 분석 목적으로 유지.
- **기본 전략**: 시작 시 프로파일/설정의 `strategy` 값으로 전략을 구성한다. 미지정 시 기본값은 `BNH`(가장 보수적·무의존). `start_trading` 시점에 전략이 구성되어 있지 않으면 에러 대신 기본 전략으로 구성 후 시작하고 그 사실을 응답에 명시한다.
...
- Controller는 시작 시 `--profile NAME` 또는 `--config PATH`로 프로파일을 로드해 초기 구성을 만든다. `--strategy CODE` CLI 플래그를 신설해 개별 플래그 조합으로도 전략 선택 가능.
- 하위 호환: 기존 `config/virtual-upbit.json` 형식과 개별 CLI 플래그(`--budget` 등)는 계속 동작. 프로파일은 그 위의 선택적 번들. `strategy` 키가 없는 기존 설정 파일은 기본 전략(`BNH`)으로 동작.
