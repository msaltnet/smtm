...
- **포함**: LLM 기반 매매 오케스트레이션, 안전장치, 시장 데이터 / 거래소 연동, 대화형 제어(CLI · Telegram · Jupyter), 관찰 가능성(로그).
...
- **[MVP] R-CHAT-01** — CLI 프로세스는 표준 입력으로 사용자 메시지를 받고, LLM 응답을 표준 출력에 출력해야 한다.
- **[MVP] R-CHAT-02** — `start` / `stop` 명령은 자동 매매 타이머를 on/off 해야 하며, 대화 세션 자체는 유지돼야 한다.
- **[MVP] R-CHAT-03** — `q` / `quit` / `exit` / `terminate` 입력 시 프로세스를 안전하게 종료해야 한다(진행 중 작업은 완료까지 대기).
...
- **[MVP] R-ORCH-02** — 자동 매매 중에는 `--term` 주기(기본 60초)마다 LLM에 시장 컨텍스트를 포함한 프롬프트를 보내 판단을 받아야 한다.
...
- **[MVP] R-SAFE-03** — 기본값은 `max_trade_amount=100_000`, `max_daily_trades=20`, `max_loss_ratio=-0.20`, `initial_budget=--budget` 값이다.
...
- **[후속] R-SAFE-07** — 한도를 CLI 옵션 또는 환경변수로 노출.
...
- **[MVP] R-CFG-01** — CLI는 `--mode`, `--budget`, `--currency`, `--exchange`, `--term`, `--token`, `--chatid`, `--log`, `--version` 옵션을 지원한다.
- **[MVP] R-CFG-02** — 민감 정보는 환경변수 또는 `.env`로만 받는다(코드·커밋 금지).
- **[MVP] R-CFG-03** — `--mode` 미지정 시 도움말을 출력하고 종료한다.
...
- [ ] 거래소 키 누락 시 `execute_trade`에서 명확한 실패 메시지가 반환되는가
- [ ] `--exchange BNC`처럼 Trader가 없는 코드 지정 시 프로세스가 안전하게 중단되는가
- [ ] `--version`이 현재 `__version__`과 일치하는가

### 4.2 대화
- [ ] CLI에서 `start` → 60초 주기로 LLM 호출 로그가 확인되는가
