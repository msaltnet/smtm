...
"Factory 등록"된 Provider는 `--exchange <CODE>`로 CLI에서 바로 선택할 수 있고, 그 외는 코드에서 직접 `from smtm import ...`로 사용합니다.
...
Binance USDT-M 선물의 주요 포지셔닝 지표. `--currency`(BTC/ETH/…)를 기준으로 `{SYMBOL}USDT` 심볼이 자동 조립됩니다. 모든 Provider가 존재하지 않는 심볼에 대해 빈 리스트를 반환합니다.
...
여러 소스를 한 번의 `get_info()` 응답으로 합쳐 주는 오케스트레이션 Provider입니다. `--exchange <CODE>`로 바로 선택할 수 있고, 생성자에 `providers=[...]` 리스트를 주입해 구성을 직접 바꿀 수도 있습니다.
...
> `UFC`는 가장 무겁습니다. 틱마다 토큰이 크게 증가하므로 `--term`을 길게(예: 300초 이상) 잡거나 실험·연구 용도로 사용하세요.
...
|------|-------------------------|----------|
| Upbit / Bithumb / Binance | 공개 티어 기준 초당 수회~수십회 | `--term` 늘리기 |
| CoinGecko 무료 티어 | 분당 ~30회 | `--term`을 60초 이상으로 유지 |
...
| CoinGecko Global | CoinGecko 무료 티어 공유(분당 ~30회) | `--term` 60초+ 유지 |
...
| Etherscan (무키) | IP당 5 req/sec 공유 | 무키면 `--term` 60초+, 키 있으면 `ETHERSCAN_API_KEY` 설정 |
...
4. CLI 노출이 필요하면 `DataProviderFactory.DataProvider_LIST`에 등록하고 `--exchange {CODE}`로 바로 선택.
