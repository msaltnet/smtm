...
A. `term` 설정값(기본 60초) 주기로 매매 루프가 돕니다. `LLM` 전략을 쓰면 틱마다 시스템 프롬프트 + 시장 데이터 + 대화 이력을 전송하므로 장시간 구동 시 비용이 누적됩니다. 채팅으로 프로파일의 `term`을 늘리거나 알고리즘 전략(BNH/RSI/SMA)을 쓰면 비용을 줄일 수 있습니다. `SystemMonitor.get_llm_usage()`로 누적 토큰을 추적하세요.
...
**Q. 한도를 바꿀 수 있나요?**
A. 명령행 플래그로는 못 바꾸고, 채팅으로 프로파일의 `safety` 설정값을 지정하면 됩니다. 코드 레벨 기본값은 `SystemOperator` 생성 시 `config["safety"]` dict로 주입합니다 (예시는 [user-guide.md 시나리오 6](user-guide.md#시나리오-6--안전장치-기본값이-너무-타이트해서-조정) 참고).
...
A. `UpbitFullContextDataProvider`(`UFC`)를 사용하세요. Upbit 캔들에 위 빌딩 블록을 전부 붙인 '풀 컨텍스트' 복합 Provider로, 프로파일의 `exchange` 설정값을 `UFC`로 지정하면 바로 선택됩니다. 틱마다 토큰이 크게 늘어나므로 연구·실험용으로 적합하고 실전에서는 `term` 설정값을 충분히 길게(예: 300초) 조절하는 것을 권장합니다. 소스 구성을 직접 지정하려면 `UpbitFullContextDataProvider(..., providers=[...])`로 리스트를 넣으면 됩니다.
...
1. 텔레그램으로 `start`를 보냈는지 확인 (보내지 않으면 타이머 미기동)
2. 세션의 `term` 설정값이 너무 큰 것은 아닌지 (기본 60초)
3. `log/smtm.log` 확인 — 매매 루프/LLM 호출 로그가 찍히는지
4. `SMTM_LLM_API_KEY`가 유효한지 (Anthropic 콘솔에서 잔액/권한 확인)
5. `default` 세션은 가상거래이므로 거래소에는 주문이 나가지 않습니다 — 실거래를 원하면 채팅으로 계좌를 등록하고 실거래 세션을 만들어야 합니다
...
5. 거래 페어(`currency` × `exchange` 설정값)가 실제로 존재하는지
6. 실거래 세션인지 (가상거래 세션의 주문은 거래소로 나가지 않습니다)
...
1. 채팅으로 프로파일의 `term` 설정값을 늘려 틱 주기를 길게 (예: 300초)
2. 동시에 돌리는 세션 수를 줄이기 (세션마다 독립적으로 LLM/거래소를 호출함)
3. 알고리즘 전략(`BNH`/`RSI`/`SMA`)을 쓰면 매매 루프에서 LLM을 호출하지 않음
4. `context.candle_count`를 줄여 매 틱당 전달되는 시장 데이터 축소
5. `max_conversation_turns`를 줄여 전송 이력 축소
6. 사용자 메시지 길이가 길면 시스템 프롬프트보다 사용자 메시지가 토큰을 더 먹을 수 있음 — 간결하게 대화
