...
class TradingRequest:
    ...
    def __init__(self, trading_type=None, price=0, amount=0):
        self.__id = self.id = str(round(time.time(), 3))
        self.__type = self.type = trading_type
        self.__price = self.price = price
        self.__amount = self.amount = amount
        self.__timestamp = time.time()

        if trading_type is not None and price > 0 and amount > 0:
    ...
    def set_info(self, trading_type=None, price=0, amount=0):
        """거래 유형, 가격, 수량을 설정한다"""
        if self.__is_fixed:
            return

        if self.__type is None and trading_type is not None:
            self.__type = self.type = trading_type
    ...
    def is_stained(self):
        ...
        return (
            ...
            or self.__price != self.price
            or self.__amount != self.amount
            or self.__id != self.id
        )
