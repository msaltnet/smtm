...
채팅으로 제어하는 LLM 에이전트가 전략 선택, 매매 시작/중지, 계좌 프로파일 관리를 담당하고, 실제 매매는 별도의 고정 주기 루프가 수행합니다.

1. SystemOperator(채팅 에이전트)가 Tool을 통해 전략을 선택/전환하고 매매를 시작/중지  
2. TradingOperator가 고정 주기 루프를 실행: DataProvider -> Strategy -> SafetyGuard -> Trader -> Analyzer  
3. Strategy는 교체 가능 — 알고리즘 전략(Buy & Hold, RSI, SMA) 또는 매 틱 LLM 판단 1회(`LLM`)  
4. SafetyGuard가 모든 주문 전에 거래 제한을 강제하고, SystemMonitor가 독립적으로 모든 활동을 기록  

## 주요기능
- 채팅 기반 오케스트레이션 에이전트: 전략 선택, 매매 시작/중지, 계좌 프로파일 관리
...
지원하는 설정 키는 `mode`, `budget`, `currency`, `exchange`, `virtual`, `paper`, `term`, `log`, `token`, `chatid`입니다. 가상거래에는 `virtual` 사용을 권장하며, `paper`는 호환용 별칭으로 계속 지원합니다. `interval`은 `term`의 별칭으로, `chat_id`는 `chatid`의 별칭으로 사용할 수 있습니다. CLI 인자를 함께 주면 설정 파일 값보다 우선합니다.
