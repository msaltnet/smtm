...
가상거래 모드는 선택한 DataProvider는 그대로 쓰되, 실제 거래소로 주문을 전송하지 않고 인메모리 `SimulationTrader`의 가상계좌에서 매매를 처리합니다. 그래서 가상 잔고, 보유 자산, 포트폴리오 가치, 수익률을 확인할 수 있습니다. 현재가는 최신 `primary_candle` 종가 시세입니다.
